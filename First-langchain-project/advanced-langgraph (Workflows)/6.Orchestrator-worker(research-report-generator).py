...
class ResearchPlan(BaseModel):
  ...
  reasoning:str = Field(
    description="Brief explaination of why these sources were chosen"
  )
...
def research_worker(state:WorkerState)->OverallState:
  ...
  prompt = f"""You are a spcialied researcher investigating: {state['research_topic']}
  ...
  - How does it rlate to the broader topic?
  ...
  Be specific, factual, and provide depth on ths particular aspect."""
...
def synthesize_report(state:OverallState)->OverallState:
  ...
  You have recieved detailed findings from {len(state['worker_findings'])} specialized researchers, each
  investigating a different aspect of this topic.

  RESEARCH FINDINGS:
  {all_findings}

  Create a cohesive, well-structured research report (500-700 words)
  ...
  2. INTRODUCTION
  - Context and importance of thi topic
  ...
  - Identify patterns accross different research areas
  ...
  IMPORTANT:
  - Write this as a unified, flowing report, NOT as seperate sections
...
def create_research_workers(state:OverallState):
  ...
  print("DISPATCHER: Creating research workers dynamcilly...")
...
print("="*70)
print(f"FINAL SYTHESIZE RESULT")
print("="*70)
